...
proprietary backbone.  The language-model pilot is in ``creoh_llm_pilot.py``.
"""
...
from creoh_routing import (TriangularFuzzyNumber, ExpectedCost, OWARisk,
                            ScenarioStability, FitnessVector, ParetoArchive, p95,
                            cliffs_delta, holm, dial_scores, tail_count)
...
class ScheduleGenerator:
    ...
    def generate(self, seed: int) -> FuzzyScheduleInstance:
        ...
        # zone volatility nu_g ~ U(0.05, 0.85) * (delta / 0.15), identical to the
        # routing and public families (fuzzy-injection protocol of the manuscript)
...
class LatinHypercubeBuilder(ScenarioBuilder):
    ...
    def build(self, inst, seed):
        ...
        # Latin hypercube: each zone visits every stratum once, independently
        perms = [rng.permutation(self.m) for _ in range(Z)]
        mats = []
...
class Genome:
    ...
    balance: float = 0.0    # > 0.5: worst-fit (balance loads); else best-fit (tight)
...
class DispatchHeuristic:
    ...
    def assign(self, inst: FuzzyScheduleInstance) -> list[list[int]]:
        ...
        order = np.argsort(-inst.proc, kind="stable")
        machines: list[list[int]] = []
        loads: list[float] = []
        worst_fit = self.g.balance > 0.5
        for j in order:
            ...
            for k, ld in enumerate(loads):
                if ld + size <= cap:
                    slack = cap - (ld + size)
                    # worst-fit keeps loads balanced; best-fit packs tightly
                    score = -slack if worst_fit else slack
                    if score < best_score:
                        best, best_score = k, score
            if best == -1:
                machines.append([j]); loads.append(size)
            else:
                machines[best].append(j); loads[best] += size
        return machines

    def cost_vector(self, inst, scenarios: np.ndarray,
                    machines: list[list[int]] | None = None) -> np.ndarray:
        machines = self.assign(inst) if machines is None else machines
...
class Candidate:
    ...
    train: np.ndarray
    ood: np.ndarray
    nominal_imbalance: float = 0.0   # MAD of nominal technician loads / shift
...
class CandidatePool:
    ...
    def grow(self, inst, train, ood, seed) -> list[Candidate]:
        ...
        for b in range(self.budget):
            ...
            mach = h.assign(inst)
            loads = np.array([inst.proc[m].sum() for m in mach])
            imb = float(np.abs(loads - loads.mean()).mean() / inst.shift)
            pool.append(Candidate(g, h.cost_vector(inst, train, mach),
                                  h.cost_vector(inst, ood, mach), imb))
        return pool
...
class ClassicalMethod(Method):
    name = "Classical heuristic"   # LPT best-fit packing at the shift (pack=1, no buffer)
...
class MONoFuzzyMethod(Method):
    ...
    def select(self, pool):
        # nominal objectives only: modal cost and nominal load imbalance
        arch = ParetoArchive()
        for c in pool:
...
def run(seeds: int = 30, n: int = 40, budget: int = 120, orness: float = 0.7,
    ...
    headline = dict(seeds=seeds, n_tasks=n, budget=budget, orness=orness,
                    ...
                    tail_count_M40=tail_count(orness, 40),
                    ...
                    stability_reduction_pct=round(100 * (det_f3 - cr_f3) / det_f3, 2),
    ...
    print(f"Stability reduction    : {headline['stability_reduction_pct']:5.1f}%")
