...
Generates computed benchmark outputs for the central scientific claim of the manuscript:
selecting heuristic programs with a fuzzy multi-objective evaluator
(expected cost / OWA tail risk / inter-scenario stability) yields more
...
backbone.  The language-model pilot is a separate module
(``creoh_llm_pilot.py``) that re-executes released programs.
"""
...
class LatinHypercubeBuilder(ScenarioBuilder):
    ...
    def build(self, inst: FuzzyInstance, seed: int) -> np.ndarray:
        ...
        # Latin hypercube: every zone visits every stratum exactly once, in an
        # independent random order, so zones are stratified but not co-moving.
        perms = [rng.permutation(self.m) for _ in range(G)]
        mats = []
        for k in range(self.m):
            shocks = np.array([rng.uniform(cuts[perms[g][k]], cuts[perms[g][k] + 1])
                               for g in range(G)])
...
class RoutingHeuristic:
    ...
    def cost_vector(self, inst: FuzzyInstance, scenarios: np.ndarray,
                    routes: list[list[int]] | None = None) -> np.ndarray:
        routes = self.build_routes(inst) if routes is None else routes
...
def tail_count(theta: float, M: int) -> int:
    ...
    integer cutoff.
    """
    return max(1, int(math.ceil((1.0 - theta) * M - 1e-9)))
...
class OWARisk(Objective):
    ...
    def __call__(self, c):
        ...
        if M == 1:
            return float(c[0])
        q = tail_count(self.orness, M)
        worst = np.sort(c)[::-1][:q]
        return float(worst.mean())


class ScenarioStability(Objective):
    """Inter-scenario stability f3: mean absolute deviation of the scenario costs.

    The quantity is *not* workforce fairness; see the terminology remark in
    the manuscript.
    """
...
# --------------------------------------------------------------------------
# Planner dial: the single selection rule used by every C-R-EoH component
# --------------------------------------------------------------------------
def dial_scores(fvs, theta: float, use_tail: bool = True) -> np.ndarray:
    """Dial score of eq. (dial) in the manuscript, lower is better.

    ``f1`` and ``f2`` are min-max normalised over the candidate set passed in
    (the archive, or the whole pool for the no-archive ablation) and combined
    as ``(1-theta) n1 + theta n2``.  Stability ``f3`` acts through archive
    dominance, not through the score.  With ``use_tail=False`` the score is
    ``n1`` alone.
    """
    vals = np.array([[fv.f1, fv.f2] for fv in fvs], dtype=float)
    norm = (vals - vals.min(0)) / (np.ptp(vals, 0) + 1e-9)
    if not use_tail:
        return norm[:, 0].copy()
    return (1 - theta) * norm[:, 0] + theta * norm[:, 1]
...
class ParetoArchive:
    ...
    def hypervolume(self, ref: tuple[float, float]) -> float:
        ...
        hv, prev_y = 0.0, ref[1]
        for x, y in nd:                        # x increasing, y decreasing
...
class Candidate:
    ...
    nominal_imbalance: float = 0.0   # MAD of nominal route loads / capacity
...
class CandidatePool:
    ...
    def grow(self, inst, train_scn, ood_scn, seed) -> list[Candidate]:
        ...
        for b in range(self.budget):
            ...
            routes = h.build_routes(inst)
            loads = np.array([inst.demand[r].sum() for r in routes], dtype=float)
            imb = float(np.abs(loads - loads.mean()).mean() / inst.capacity)
            cand = Candidate(g, h.cost_vector(inst, train_scn, routes),
                             h.cost_vector(inst, ood_scn, routes), imb)
            pool.append(cand)
        return pool
...
class ClassicalMethod(Method):
    ...
    def select(self, pool):
        ...
        cand = pool[0]
        arch = ParetoArchive()
        arch.add(cand.fitness(ExpectedCost(), OWARisk(), ScenarioStability()), cand)
        return cand, arch
...
class DeterministicMethod(Method):
    ...
    def select(self, pool):
        ...
        arch = ParetoArchive()
        arch.add(best.fitness(ExpectedCost(), OWARisk(), ScenarioStability()), best)
        return best, arch
...
class MONoFuzzyMethod(Method):
    ...
    def select(self, pool):
        # multi-objective selection WITHOUT fuzzy scenarios: both objectives are
        # evaluated on the modal instance only -- nominal cost and nominal
        # route-load imbalance -- and the same dial rule picks the member, so
        # the alpha-cut tail never enters selection.
        arch = ParetoArchive()
        for c in pool:
            arch.add(FitnessVector(c.train[0], c.nominal_imbalance, 0.0), c)
        fvs = [fv for fv, _ in arch.items]
        k = int(np.argmin(dial_scores(fvs, self.orness)))
        return arch.items[k][1], arch
...
class CREoHMethod(Method):
    ...
    def select(self, pool):
        ...
        # OWA risk-dial selection over the archive (eq. dial of the manuscript)
        fvs = [fv for fv, _ in arch.items]
...
def run(seeds: int = 30, n: int = 25, budget: int = 80, orness: float = 0.7,
    ...
    for seed in range(seeds):
        ...
        for m in methods:
            ...
            # (f1, f2). Stability remains part of archive dominance and
