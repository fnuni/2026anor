...
def metadata():
    ...
    return dict(provider='Qwen via local Ollama', model=MODEL,
                ...
                tools='none supplied', api_logs=True, fine_tuned=False, thinking=False,
                hardware=platform.platform() + ' ' + platform.machine(),
                reflection_blocks='drafted by assisting AI from actual training outcomes; archived verbatim',
...
def generate(root, generation, count, feedback=None):
    ...
    for i in range(1, count + 1):
        ...
        request = dict(model=MODEL, stream=False, think=False,
